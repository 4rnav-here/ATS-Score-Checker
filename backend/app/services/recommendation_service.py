...
v2 improvements (recommendationsystemimprovement.md):
    - Skill-gap recommendations now include SuggestedContent:
      • Skills section keyword string  → paste directly into Skills
      • 1–2 Experience/Projects bullets → paste into the right section
    - Each recommendation carries suggested_content: list[SuggestedContent]
    - Content is generated by content_generator_service (template → LLM fallback)
"""
...
def _build_suggested_content(content_data: dict) -> list[dict]:
    """
    Convert a content_generator_service result dict into a list of
    SuggestedContent-compatible dicts.
    """
    suggestions = []

    # Skills section entry
    if content_data.get("skills_entry"):
        suggestions.append({
            "section": "Skills",
            "label": f"Add to Skills section",
            "content": content_data["skills_entry"],
            "content_type": "skill_keyword",
        })

    # Experience / Projects bullets
    placement = content_data.get("placement", "Experience")
    for bullet in content_data.get("bullets", []):
        suggestions.append({
            "section": placement,
            "label": f"Add bullet to {placement} section",
            "content": bullet,
            "content_type": "bullet",
        })
...
def generate_recommendations(
    ...
    resume_sections: dict | None = None,   # NEW — parsed sections for context
) -> list[dict]:
    """
    ...
    Each recommendation is a dict with keys:
        ...
        suggested_content — list of SuggestedContent dicts (new in v2)
    ...
    # ── 5. Skill gap recommendations (v2: with generated content) ─────────────
    if skill_gaps:
        # Generate content for each gap (template first, LLM fallback)
        generated: list[dict] = []
        if resume_sections:
            try:
                from app.services.content_generator_service import generate_content_for_gaps
                generated = generate_content_for_gaps(
                ...
                pass  # Degrade gracefully — still emit text-only recommendations

        # Emit one recommendation per generated skill (or one bulk rec if no content)
        if generated:
            for content_data in generated:
                skill = content_data["skill"]
                suggested = _build_suggested_content(content_data)
                source_label = "" if content_data["source"] == "template" else " (AI-generated)"
                recs.append({
                    "priority": PRIORITY_HIGH if len(skill_gaps) > 5 else PRIORITY_MEDIUM,
                    "category": CAT_SKILL,
                    "title": f"Add '{skill}' to pass this ATS keyword filter",
                    "description": (
                        f"'{skill}' appears in the job description but is absent from your resume. "
                        f"Below are ready-to-paste additions{source_label} — use them only if "
                        "you have genuine experience with this skill."
                    ),
                    "impact": "Improves both keyword and semantic scores",
                    "suggested_content": suggested,
                })
        else:
            # Fallback text-only bulk recommendation
            top_skills = ", ".join(skill_gaps[:8])
            recs.append({
                "priority": PRIORITY_HIGH if len(skill_gaps) > 5 else PRIORITY_MEDIUM,
                "category": CAT_SKILL,
                "title": f"{len(skill_gaps)} skill gap(s) detected",
                "description": (
