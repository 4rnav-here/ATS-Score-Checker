...
async def run():
    pdf_path = "../Scandid_Resume.pdf"
    if not os.path.exists(pdf_path):
        # Fallback to Arnav Trivedi.pdf if Scandid_Resume is not present
        pdf_path = "../Arnav Trivedi.pdf"
        if not os.path.exists(pdf_path):
            print("Error: Could not find Scandid_Resume.pdf or Arnav Trivedi.pdf in the root directory.")
            return
    ...
    with open(pdf_path, "rb") as f:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            try:
                response = await client.post("http://localhost:8000/api/analyze", files=files, data=data)
    ...
    assert "skills" in result["sections_found"],     "FAIL: Skills section not detected"
    assert "experience" in result["sections_found"], "FAIL: Experience section not detected"
