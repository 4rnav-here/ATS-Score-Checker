...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        ...
        router.push("/analyze"); // Go back if no data found
      }
    } catch (e) {
  ...
  const { scores, section_scores, skill_gaps, format_issues, quality_issues, jd_alignment_warning, recommendations, recommendation_summary } = data;
  ...
  return (
    <main className="flex-1 flex flex-col min-h-screen p-4 md:p-8 max-w-5xl mx-auto w-full relative z-10">
      ...
      <nav className="w-full flex items-center justify-between mb-8 text-[10px] md:text-xs uppercase tracking-widest text-secondary">
        <Link href="/analyze" className="hover:text-primary transition-colors">← NEW ANALYSIS</Link>
      ...
      <div className="mb-12 border border-borders rounded-[4px] p-6 md:p-10 flex flex-col md:flex-row items-center justify-between gap-8">
        <div>
          <h1 className="text-3xl md:text-4xl font-display font-bold uppercase tracking-tight text-primary mb-2">Analysis Complete</h1>
          <div className="w-12 h-[1px] bg-accent mb-4"></div>
          {jd_alignment_warning ? (
             <p className="text-[#ff5555] text-sm uppercase tracking-wider">{jd_alignment_warning}</p>
          ) : (
             <p className="text-secondary text-sm uppercase tracking-wider">Document alignment meets baseline thresholds.</p>
          )}
        </div>
        <div className="flex flex-col items-center justify-center border-4 border-accent rounded-full w-32 h-32 shrink-0">
          <span className="text-4xl font-display font-bold text-primary">{Math.round(scores.final)}</span>
          <span className="text-[10px] uppercase tracking-widest text-secondary">FINAL SCORE</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        ...
        <div className="flex flex-col gap-8">
          ...
          <section>
            <h2 className="text-xs uppercase tracking-widest text-secondary font-bold mb-4 border-b border-dividers pb-2">Score Breakdown</h2>
          ...
          <section>
            <h2 className="text-xs uppercase tracking-widest text-secondary font-bold mb-4 border-b border-dividers pb-2">Section Scores</h2>
            <div className="flex flex-col gap-3">
              ...
              {section_scores.projects !== null && section_scores.projects !== undefined && (
                <ScoreRow label="Projects" value={section_scores.projects} />
              )}
              {section_scores.summary !== null && section_scores.summary !== undefined && (
        ...
        {/* Right Column - Actionable Issues */}
        <div className="flex flex-col gap-8">
          
          <section>
            <h2 className="text-xs uppercase tracking-widest text-secondary font-bold mb-4 border-b border-dividers pb-2">Skill Gaps ({skill_gaps.length})</h2>
            {skill_gaps.length === 0 ? (
              <p className="text-sm text-borders">No major skill gaps detected.</p>
            ) : (
              <div className="flex flex-wrap gap-2">
                {skill_gaps.map((skill: string) => (
                  <span key={skill} className="text-xs border border-borders px-3 py-1 rounded-[2px] text-primary bg-background uppercase">
                    {skill}
                  </span>
                ))}
              </div>
          ...
          <section>
            <h2 className="text-xs uppercase tracking-widest text-secondary font-bold mb-4 border-b border-dividers pb-2">Quality & Format Issues</h2>
            <div className="flex flex-col gap-2">
              {[...format_issues, ...quality_issues].map((issue: string, idx: number) => (
            ...
            {recommendation_summary && (
              <div className="flex gap-3 text-[10px] uppercase tracking-widest">
                {recommendation_summary.critical > 0 && (
                  <span className="text-[#ff5555]">{recommendation_summary.critical} Critical</span>
                )}
                {recommendation_summary.high > 0 && (
                  <span className="text-[#ffaa55]">{recommendation_summary.high} High</span>
                )}
                {recommendation_summary.medium > 0 && (
                  <span className="text-accent">{recommendation_summary.medium} Medium</span>
                )}
                {recommendation_summary.low > 0 && (
                  <span className="text-secondary">{recommendation_summary.low} Low</span>
                )}
              </div>
            )}
          </div>

          <div className="w-full h-[1px] bg-dividers mb-6"></div>
              ...
              >
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="flex items-center gap-3">
                    <span className={`text-[10px] uppercase tracking-widest border px-2 py-0.5 rounded-[2px] font-bold ${PRIORITY_COLORS[rec.priority] || 'text-secondary border-borders'}`}>
                      {rec.priority}
                    </span>
                    <h3 className="text-sm font-bold text-primary">{rec.title}</h3>
                  </div>
                </div>
                <p className="text-sm text-secondary leading-relaxed mb-3">{rec.description}</p>
      ...
      <div className="mb-12">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-secondary text-xs mt-1 uppercase tracking-widest">
              Posted in the last 3 days · Based on your skills
            </p>
          </div>
          {!jobsFetched && (
            <button
              onClick={async () => {
                ...
                try {
                  const res = await fetch("http://localhost:8000/api/jobs/search", {
                    ...
                    body: JSON.stringify({ analysis_id: params.id, max_days_old: 3 }),
                  });
          ...
          {jobsFetched && (
            <span className="text-xs text-secondary uppercase tracking-widest">{jobs.length} Results</span>
          )}
        </div>
        ...
        {jobsLoading && (
          <div className="flex items-center gap-3 text-secondary text-xs uppercase tracking-widest py-8">
            <span className="animate-pulse">● QUERYING ADZUNA + REMOTIVE...</span>
          </div>
        )}
        ...
        {!jobsFetched && !jobsLoading && (
          <div className="border border-borders border-dashed rounded-[4px] p-8 text-center">
            <p className="text-secondary text-sm">Click the button above to fetch jobs matching your skills from the last 3 days.</p>
          </div>
        )}

        {jobsFetched && jobs.length === 0 && (
          <div className="border border-borders rounded-[4px] p-6 text-center">
            <p className="text-secondary text-sm">No matching jobs found in the last 3 days. Try again tomorrow.</p>
          </div>
        )}
        ...
        {jobs.length > 0 && (
          <div className="flex flex-col gap-4">
            {jobs.map((job: any, idx: number) => (
              <div key={idx} className="border border-borders rounded-[4px] p-5 hover:border-accent transition-colors group">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <span className="text-[10px] uppercase tracking-widest text-borders">{job.source}</span>
                    </div>
                    <h3 className="text-base font-bold text-primary group-hover:text-accent transition-colors">{job.title}</h3>
                    <div className="flex items-center gap-3 mt-1 text-xs text-secondary">
                      <span>{job.company}</span>
                      {job.location && <><span className="text-borders">·</span><span>{job.location}</span></>}
                    </div>
                  </div>
                  <a
                    href={job.url}
                ...
                {job.description && (
                  <p className="text-xs text-borders leading-relaxed line-clamp-2">{job.description}</p>
                )}
              </div>
            ))}
          </div>
...
function ScoreRow({ label, value, isPenalty = false }: { label: string, value: number, isPenalty?: boolean }) {
  return (
    <div className="flex items-center justify-between text-sm">
      <span className="text-primary">{label}</span>
      <span className={`font-mono ${isPenalty && value < 0 ? 'text-[#ff5555]' : 'text-accent'}`}>
        {isPenalty ? "" : "+"}{typeof value === 'number' ? value.toFixed(1) : value}
      </span>
    </div>
  );
}
